...
class TMDbSearch extends React.Component {
  ...
  async onSelect(event) {
    ...
    const URL = `https://db.mycoll.skybur.io/api/v1/movies/search/details?query=${movie_id}`;
  ...
  async requestResults(event) {
    ...
    const URL = `https://db.mycoll.skybur.io/api/v1/movies/search?query=${this.props.query}`;
    const axiosConfig = {
      headers: {
        Authorization: `Bearer ${this.props.accessToken}`
      }
